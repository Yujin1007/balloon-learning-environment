...
def register_env() -> None:
  ...
  envs = [
      ('BalloonLearningEnvironment-v0', 'balloon_learning_environment.env.balloon_env:BalloonEnv'),
      ('BalloonNavEnvironment-v0', 'balloon_learning_environment.env.balloon_env:BalloonNavEnv'),
  ]

  for env_id, env_entry_point in envs:
    registered = env_id in registration.registry.env_specs
    if not registered:
      with contextlib.ExitStack() as stack:
        if hasattr(registration, 'namespace'):
