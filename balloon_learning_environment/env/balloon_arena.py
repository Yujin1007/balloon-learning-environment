...
class BalloonNavArena(BalloonArenaInterface):
  """A BalloonArena in which a balloon flies in some wind field."""
  ...
  def reset(self, seed: Union[int, jnp.ndarray, None] = None) -> np.ndarray:
    ...
    self.moving_station_dx = units.Distance(kilometers=200*math.cos(direction))
    self.moving_station_dy = units.Distance(kilometers=200*math.sin(direction))
    self._atmosphere.reset(atmosphere_key)
    start_date_time = sampling.sample_time(time_key)
    self._balloon = self._initialize_balloon(start_date_time)
    assert self._balloon.state.status == balloon.BalloonStatus.OK
  ...
  def step(self, action: control.AltitudeControlCommand) -> np.ndarray:
    ...
    self._balloon.simulate_step(wind_vector, self._atmosphere, action,
                                self._step_duration)
    self.maybe_regenerate_wind_field_near_station()
    # At the end of the cycle, make a measurement, and construct features.
    self.feature_constructor.observe(self.get_measurements())
    
    return self.feature_constructor.get_features()

  def maybe_regenerate_wind_field_near_station(self, threshold_km=50.0):
        """Regenerates the wind field around the balloon if close to the station.

        Args:
            station_latlng: (lat, lng) tuple of the station/target.
            threshold_km: Distance threshold in km for regeneration.
        """
        # Get balloon's current lat/lng
        old_center_latlng = self._balloon.state.center_latlng
        
        # Compute distance (assuming units.Distance is available)
        dx = self._balloon.state.x - self.moving_station_x
        ...
        if distance.kilometers < threshold_km:
            ...
            # Regenerate wind field centered at new station
            # You may need to adapt this depending on your wind field implementation
            self._rng, wind_field_key = jax.random.split(self._rng, 2)
  ...
  def _initialize_balloon(self,
    ...
    # Note: Balloon units are in Pa.
    # Sample the starting distance using a beta distribution, within 50km.
    radius = jax.random.beta(keys[0], self._alpha, self._beta).item()
    radius = units.Distance(km=50.0 * radius)
    theta = jax.random.uniform(keys[1], (), minval=0.0, maxval=2.0 * jnp.pi)
